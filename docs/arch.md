...
7. **配方编辑** → 复用添加配方页的编辑模式加载自定义配方，校验后在原自定义配方基础上更新，保留原配方 ID，刷新最近修改时间，成功后返回该配方详情页
8. **自定义配方管理** → 用户创建、查看、编辑、单条删除或批量删除当前设备上的自定义配方；新增、导入和编辑都不应产生与内置配方或自定义配方重名的配方，导入重名时建议递增名称，新增和编辑重名时提示用户修改，删除前必须二次确认；批量删除确认时显示所选数量；从详情页删除后回到自定义配方管理页，从列表删除后留在列表并刷新
